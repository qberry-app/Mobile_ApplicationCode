...
public typealias customSelectionClosure = (Index, NSAttributedString) -> Void
public typealias MultiSelectionClosure = ([Index], [String]) -> Void
public typealias ConfigurationClosure = (Index, String) -> String
public typealias CustomConfigurationClosure = (Index, NSAttributedString) -> NSAttributedString
public typealias attributedCellConfigurationClosure = (Index, NSAttributedString, DropDownCell) -> Void
...
public final class DropDown: UIView {
	...
	*/
    ...
    public var customDataSource = [NSAttributedString]() {
        didSet {
            deselectRows(at: selectedRowIndices)
            reloadAllComponents()
        }
	...
	*/
    ...
    public var attributedCellConfiguration: CustomConfigurationClosure? {
        didSet { reloadAllComponents() }
    }
    /**
    ...
    public var attributedCustomCellConfiguration: attributedCellConfigurationClosure? {
        didSet {reloadAllComponents()}
    }

	/// The action to execute when the user selects a cell.
	public var selectionAction: SelectionClosure?
    public var customSelectionAction: customSelectionClosure?
    /**
	...
	*/
    public convenience init(anchorView: AnchorView, selectionAction: SelectionClosure? = nil, dataSource: [String] = [],customDataSource: [NSAttributedString] = [], topOffset: CGPoint? = nil, bottomOffset: CGPoint? = nil, cellConfiguration: ConfigurationClosure? = nil, cancelAction: Closure? = nil) {
		self.init(frame: .zero)
...
extension DropDown {
	...
	fileprivate func fittingWidth() -> CGFloat {
        ...
        if dataSource.count == 0 {
            for index in 0..<customDataSource.count {
...
extension DropDown {
	...
	*/
	...
	public var selectedItem: String? {
        ...
        if dataSource.count == 0 {
            return customDataSource[row].string
        } else {
	...
	fileprivate var tableHeight: CGFloat {
        if dataSource.count == 0 {
            return tableView.rowHeight * CGFloat(customDataSource.count)
        } else {
...
extension DropDown: UITableViewDataSource, UITableViewDelegate {
	...
	fileprivate func configureCell(_ cell: DropDownCell, at index: Int) {
        ...
        if dataSource.count == 0 {
            if let cellConfiguration = cellConfiguration {
                cell.optionLabel.attributedText = attributedCellConfiguration!(index, customDataSource[index])
            } else {
                cell.optionLabel.attributedText = customDataSource[index]
            }
            
            attributedCustomCellConfiguration?(index, customDataSource[index], cell)
        } else {
            if let cellConfiguration = cellConfiguration {
	...
	public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectedRowIndices.insert(selectedRowIndex)
        if dataSource.count == 0 {
            customSelectionAction?(selectedRowIndex, customDataSource[selectedRowIndex])
        } else {
            selectionAction?(selectedRowIndex, dataSource[selectedRowIndex])
        }
